...
```{r, warning=F, message=F, cache=T, fig.height=10, fig.width=10}
...
``` 
    ...
    There does seem to be a clear relationship between location and January minimum temperatures.  If this were not the case,  

```{r p6_a, warning=F,message=F}
set.seed(444)

rlon <- runif(length(tc1$lon), min(tc1$lon), max(tc1$lon))
rlat <- runif(length(tc1$lat), min(tc1$lat), max(tc1$lat))
