corrplot_mg<-function (corr, method = c("circle", "square", "ellipse", "number", 
                       ...
                       uppCI.mat = NULL, ...) 
{
  ...
  ord <- 1:min.nm
  if (!order == "original") {
    ord <- corrMatOrder_mg(corr, order = order, hclust.method = hclust.method)
    corr <- corr[ord, ord]
  }
...
corrMatOrder_mg=function (corr, order = c("AOE", "FPC", "hclust", "alphabet"), 
                                            ...
                                            "mcquitty", "median", "centroid")) 
{
  ...
  if (order == "hclust") {
    ord <- order.dendrogram(as.dendrogram(hclust(as.dist(sqrt(2*(1 -abs(corr)))), method = hclust.method)))
  }
  return(ord)
}
